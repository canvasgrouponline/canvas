<?php
    ...
    // Load CSS
    function canvas_photo_load_styles() {
        // Register Styles
        wp_register_style('font-awesome_style', '//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css');
        wp_register_style('canvas_photo_main_style', get_template_directory_uri() . '/assets/styles/main.css');

        // Enqueue Styles
        wp_enqueue_style( 'canvas-style', get_stylesheet_uri() );
        wp_enqueue_style( 'font-awesome_style' );
        wp_enqueue_style( 'canvas_photo_main_style' );
    }
    add_action('wp_enqueue_scripts', 'canvas_photo_load_styles');

    // Load Javascript
    function canvas_photo_load_scripts() {
    ...
    add_action('wp_enqueue_scripts', 'canvas_photo_load_scripts');
?>
